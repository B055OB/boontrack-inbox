...
export default function BoonPilotWidget({ tenantSlug }: BoonPilotWidgetProps) {
  ...
  const initialWelcome: ChatMessage = {
    id: 'welcome-1',
    sender: 'assistant',
    text: `Halo! Saya **BoonPilot**, AI Copilot & Asisten Toko Anda. 🚀\n\nSaya siap membantu Anda memantau performa penjualan, memeriksa ketersediaan stok, hingga mengatur konfigurasi operasional toko secara otomatis.`,
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div className="mb-3.5 w-[calc(100vw-32px)] sm:w-[420px] h-[580px] max-h-[85vh] bg-white rounded-3xl shadow-2xl border border-slate-200/80 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200 isolate">
          ...
          {/* Quick Starter Chips (Shown at start of session or when few messages exist) */}
          {!hasUserMessages && (
                ...
                {STARTER_CHIPS.map((chip, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full text-left p-2 rounded-xl bg-white hover:bg-blue-50/80 hover:text-blue-700 border border-slate-200/80 text-slate-700 text-xs font-semibold transition flex items-center gap-2 shadow-2xs group cursor-pointer"
                    >
